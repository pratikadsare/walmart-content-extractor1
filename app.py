...
FIELD_ALIASES = {
    "gender": ["gender"],
    "count_per_pack": ["count per pack", "countperpack"],
    "multipack": ["multipack", "multi pack", "pack", "pack size"],
    "total_count": ["total count", "totalcount", "count", "quantity"],
    "size": ["size"],
    "serving_size": ["serving size", "servingsize"],
    "color": ["color", "colour"],
    "flavour": ["flavor", "flavour", "taste"],
    "product_form": ["product form", "form"],
    "ingredient_statement": ["ingredient statement", "ingredients", "ingredient"],
    "dosage": ["dosage"],
    "directions": ["directions", "direction"],
    "instructions": ["instructions", "instruction"],
    "stop_use_indications": ["stop use indications", "stop use indication", "warning", "warnings"],
    "health_concern": ["health concern", "health concerns"],
}
...
def is_probable_image_url(value: str) -> bool:
    ...
    if 'walmartimages' not in text_lower and not re.search(r'\.(jpg|jpeg|png|webp)(\?|$)', text_lower):
        return False
    blocked = ['sprite', 'logo', 'icon', 'placeholder', 'spinner']
    return not any(token in text_lower for token in blocked)
...
def walk_json_object(
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            walk_json_object(value, candidate_map, image_urls, normalize_label(str(key)))
        return
    if isinstance(obj, list):
        for item in obj:
            walk_json_object(item, candidate_map, image_urls, current_key=current_key)
        return
    if obj is None:
        return
...
def lookup_candidate(candidate_maps: Iterable[Dict[str, List[str]]], aliases: Iterable[str], prefer_long: bool = False) -> str:
    ...
    for alias in normalized_aliases:
        for candidate_map in candidate_maps:
            values = candidate_map.get(alias, [])
            if values:
                return max(values, key=len) if prefer_long else values[0]
    for alias in normalized_aliases:
        for candidate_map in candidate_maps:
...
def extract_image_links(soup: BeautifulSoup, html: str, json_image_urls: List[str]) -> List[str]:
    ...
    for tag in soup.find_all(['meta', 'img', 'source']):
        for attr in ['content', 'src', 'data-src', 'srcset', 'data-image-src']:
            raw = tag.get(attr)
            if not raw:
                continue
            parts = [raw]
            ...
            for part in parts:
                url = canonicalize_url(part)
                if is_probable_image_url(url):
                    image_urls.append(url)
    regex_urls = re.findall(r'https?:\\?/\\?/[^"\'\s>]+', html, flags=re.I)
    regex_urls += re.findall(r'https?://[^"\'\s>]+', html, flags=re.I)
    for raw in regex_urls:
        url = canonicalize_url(raw)
        if is_probable_image_url(url):
            image_urls.append(url)
    image_urls.extend(json_image_urls)
    filtered: List[str] = []
    for url in dedupe_keep_order(image_urls):
        lower_url = url.lower()
        if any(token in lower_url for token in ['sprite', 'logo', 'placeholder', 'icon', 'spinner']):
            continue
        filtered.append(url)
    return filtered
...
def extract_field_value(
    ...
    if field_key in SECTION_PATTERNS:
        section_text = extract_section_text(lines, SECTION_PATTERNS[field_key], max_scan_lines=18)
    ...
    if need_description or need_scalar_fields:
        description = first_non_empty(
            [
                ...
                lookup_candidate([json_candidates], ['description', 'short description'], prefer_long=True),
...
def render_login_page() -> None:
    ...
    left, center, right = st.columns([1.15, 1.15, 1.15])
    with center:
...
def render_dashboard() -> None:
    ...
    with left:
        with st.container(border=True):
            ...
            action_col1, action_col2, action_col3 = st.columns([1.3, 1.0, 1.4])
            with action_col1:
