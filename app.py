...
def infer_total_count_from_text(text: str) -> str:
    candidate = clean_text(text)
    match = re.search(
        r'\b(\d+(?:\.\d+)?)\s*(count|ct|gummies|tablets|capsules|softgels|chews|pieces|servings?)\b',
        candidate,
        flags=re.I,
    )
    if match:
...
def infer_product_form_from_title(text: str) -> str:
    candidate = clean_text(text)
    for form in PRODUCT_FORM_CHOICES:
        if re.search(rf'\b{re.escape(form)}\b', candidate, flags=re.I):
            return form
    return ''
...
def sanitize_simple_choice(value: str) -> str:
    ...
    if not text or looks_like_bad_value(text):
        return ''
    text = re.sub(r'\s*selected,\s*', '', text, flags=re.I)
    text = re.sub(r',\s*\$[^,]+$', '', text)
    text = re.sub(r'\s+', ' ', text).strip(' ,')
...
def sanitize_size_value(value: str) -> str:
    text = sanitize_simple_choice(value)
    if not text:
        return ''
    match = re.search(r'(\d+(?:\.\d+)?\s*(?:count|ct|capsules?|tablets?|gummies|softgels?|pieces|servings?)(?:\s*\(pack of \d+\))?)', text, flags=re.I)
    if match:
        return clean_text(match.group(1))
    return text if len(text.split()) <= 8 else ''


def sanitize_count_value(value: str) -> str:
    text = sanitize_simple_choice(value)
    if not text:
        return ''
...
def sanitize_field_value(field_key: str, value: str, title: str = '', description: str = '') -> str:
    ...
    if not text or looks_like_bad_value(text):
        return ''
    if field_key in {'gender', 'color', 'flavour'}:
        text = sanitize_simple_choice(text)
        if len(text.split()) > 6:
            return ''
        return text
    if field_key == 'product_form':
        text = sanitize_simple_choice(text)
        inferred = infer_product_form_from_title(text) or infer_product_form_from_title(title)
        return inferred or (text if text in PRODUCT_FORM_CHOICES else '')
    if field_key == 'size':
        return sanitize_size_value(text)
    if field_key == 'count_per_pack':
        count = sanitize_count_value(text)
        return count
    if field_key == 'multipack':
        return infer_multipack_from_text(text or title)
    if field_key == 'total_count':
        return infer_total_count_from_text(text) or infer_total_count_from_text(title) or infer_total_count_from_text(description)
    if field_key == 'serving_size':
        text = sanitize_simple_choice(text)
...
def select_best_image_urls(urls: List[str], title: str) -> List[str]:
    ...
    for raw in urls:
        ...
        if not is_probable_image_url(url):
            continue
        lower_url = url.lower()
        if any(token in lower_url for token in ['logo', 'icon', 'sprite', 'placeholder', 'rating', 'review', 'sponsored', 'related', 'similar']):
            continue
...
def extract_image_links(soup: BeautifulSoup, title: str, json_image_urls: List[str]) -> List[str]:
    ...
    hero_urls: List[str] = []
    for selector in [
        '[data-testid*="hero"] img[src*="walmartimages"]',
        '[data-testid*="media"] img[src*="walmartimages"]',
        '[data-testid*="image"] img[src*="walmartimages"]',
    ]:
        for tag in soup.select(selector):
            ...
            if alt_text and title and title_similarity_score(alt_text, title) < 0.35:
                continue
            hero_urls.append(normalize_image_url(raw))

    return select_best_image_urls(preferred_urls + hero_urls + meta_urls, title)
...
def extract_field_value(
    ...
    if field_key in SECTION_PATTERNS:
        section_text = extract_section_text(lines, SECTION_PATTERNS[field_key], max_scan_lines=28)
    candidate_value = lookup_candidate(candidate_maps, aliases, prefer_long=field_key in LONG_TEXT_FIELDS)
    value = first_non_empty([section_text, candidate_value])

    if not value and field_key == 'product_form':
        value = infer_product_form_from_title(title)
    if not value and field_key == 'multipack':
        value = infer_multipack_from_text(title) or infer_multipack_from_text(description)
...
def scrape_listing_from_html(html: str, selected_field_keys: List[str], resolved_url: str = '') -> ExtractionBundle:
    ...
    candidate_maps = [line_candidates, dom_candidates, json_candidates]
...
def render_login_page() -> None:
    inject_login_css()
    left, center, right = st.columns([1.8, 1.05, 1.8], gap='large')
    with center:
