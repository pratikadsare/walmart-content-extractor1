...
FIELD_ALIASES = {
    ...
    "gender": ["gender"],
    "count_per_pack": ["count per pack", "countperpack", "pieces per pack", "units per pack", "item package quantity", "package quantity"],
    "multipack": ["multipack", "multi pack", "number of packs", "pack quantity"],
    "total_count": ["total count", "totalcount", "count", "item count", "unit count"],
    ...
    "product_form": ["product form", "item form", "form factor"],
    "ingredient_statement": ["ingredient statement", "ingredients", "ingredient", "ingredient list"],
    "dosage": ["dosage", "dose", "suggested use"],
    "directions": ["directions", "direction"],
    "instructions": ["instructions", "instruction", "usage instructions"],
    "stop_use_indications": ["stop use indications", "stop use indication", "warnings", "warning"],
...
FIELD_REGEX_PATTERNS = {
    ...
    "product_form": [r'"productForm"\s*:\s*"(.*?)"', r'"dosageForm"\s*:\s*"(.*?)"', r'"product_form"\s*:\s*"(.*?)"'],
    "ingredient_statement": [r'"ingredientStatement"\s*:\s*"(.*?)"', r'"ingredients"\s*:\s*"(.*?)"', r'"ingredientList"\s*:\s*"(.*?)"'],
    "dosage": [r'"dosage"\s*:\s*"(.*?)"', r'"suggestedUse"\s*:\s*"(.*?)"'],
    "directions": [r'"directions"\s*:\s*"(.*?)"', r'"suggestedUse"\s*:\s*"(.*?)"'],
    "instructions": [r'"instructions"\s*:\s*"(.*?)"', r'"usageInstructions"\s*:\s*"(.*?)"'],
    "stop_use_indications": [r'"stopUseIndications"\s*:\s*"(.*?)"', r'"warnings"\s*:\s*"(.*?)"', r'"warning"\s*:\s*"(.*?)"'],
...
def extract_json_candidates_from_soup(soup: BeautifulSoup, resolved_url: str = '', page_title: str = '') -> Tuple[Dict[str, List[str]], List[str]]:
    ...
    best_record: Optional[Tuple[List[str], Dict[str, Any]]] = None
    best_score = -999.0
    for path_parts, record in candidate_records:
        score = score_product_record(path_parts, record, page_title, item_id)
        if score > best_score:
            best_score = score
            best_record = (path_parts, record)

    candidate_map: Dict[str, List[str]] = {}
    image_urls: List[str] = []
    if best_record is not None:
        _, record = best_record
        walk_product_record(record, candidate_map, image_urls)
...
def build_line_candidate_map(lines: List[str]) -> Dict[str, List[str]]:
    ...
    for key, patterns in SECTION_PATTERNS.items():
        section_text = extract_section_text(lines, patterns, max_scan_lines=28)
        if section_text:
            add_candidate(candidate_map, FIELD_LABELS.get(key, key), section_text)
    for section_patterns in ([r'^specs?$', r'^specifications?$'], [r'^ingredients?$'], [r'^directions?$'], [r'^indications?$']):
        section_pairs = extract_pairs_from_section(lines, section_patterns)
        ...
        for dt in root.select('dt'):
            ...
            if dd:
                ...
                if looks_like_label_text(label) and value:
                    add_candidate(candidate_map, label, value)
        for node in root.select('[data-testid], [aria-label], li, p, span'):
...
def select_best_image_urls(urls: List[str], title: str) -> List[str]:
    ...
    for raw in urls:
        ...
        if any(token in lower_url for token in ['logo', 'icon', 'sprite', 'placeholder', 'rating', 'review', 'sponsored', 'related', 'similar', 'variant', 'swatch', 'thumbnail']):
            continue
        if 'odnheight=117' in lower_url or 'odnwidth=117' in lower_url:
            continue
...
def extract_field_value(
    ...
    if field_key in SECTION_PATTERNS:
        section_text = extract_section_text(lines, SECTION_PATTERNS[field_key], max_scan_lines=32)
    strict_candidate = lookup_candidate(candidate_maps, aliases, prefer_long=field_key in LONG_TEXT_FIELDS, min_score=88)
    regex_value = regex_extract_first(html, FIELD_REGEX_PATTERNS.get(field_key, []))
    relaxed_candidate = lookup_candidate(candidate_maps, aliases, prefer_long=field_key in LONG_TEXT_FIELDS, min_score=72)
    if field_key in LONG_TEXT_FIELDS or field_key in {'directions', 'instructions', 'stop_use_indications'}:
        value = first_non_empty([section_text, strict_candidate, regex_value, relaxed_candidate])
    else:
        value = first_non_empty([strict_candidate, section_text, regex_value, relaxed_candidate])
...
def render_login_page() -> None:
    inject_login_css()
    left, center, right = st.columns([1.8, 1.0, 1.8], gap='large')
    with center:
