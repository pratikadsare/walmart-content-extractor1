...
FIELD_ALIASES = {
    "brand": ["brand", "brand name", "manufacturer brand", "manufacturer"],
    "gender": ["gender"],
    "count_per_pack": [
        "count per pack", "countperpack", "pieces per pack", "units per pack",
        "item package quantity", "package quantity", "quantity per pack",
    ],
    "multipack": ["multipack", "multi pack", "number of packs", "pack count", "packs per item"],
    "total_count": ["total count", "totalcount", "item count", "count", "counts", "unit count"],
    "size": ["size", "item size", "size unit", "package size"],
    "serving_size": ["serving size", "servingsize", "serving amount"],
    "color": ["color", "colour", "color family"],
    "flavour": ["flavor", "flavour", "taste"],
    "product_form": ["product form", "item form", "form factor", "form", "dosage form"],
    "ingredient_statement": [
        "ingredient statement", "ingredients", "ingredient list", "other ingredients",
        "inactive ingredients", "active ingredients",
    ],
    "dosage": ["dosage", "dose", "suggested dosage", "suggested use", "recommended dose"],
    "directions": ["directions", "direction", "suggested use", "suggested directions", "use directions"],
    "instructions": ["instructions", "instruction", "usage instructions", "use instructions"],
    "stop_use_indications": ["stop use indications", "stop use indication", "warnings", "warning", "cautions", "caution"],
    "health_concern": ["health concern", "health concerns", "health focus", "health concern(s)"],
}

FIELD_CONFLICT_TOKENS = {
    "count_per_pack": ["total count", "multipack", "multi pack", "serving size"],
    "total_count": ["count per pack", "multipack", "multi pack", "serving size"],
    "multipack": ["count per pack", "total count", "serving size"],
    "size": ["count per pack", "total count", "multipack", "multi pack", "serving size"],
    "serving_size": ["count per pack", "total count", "multipack", "multi pack", "size"],
}

FIELD_REGEX_PATTERNS = {
    "brand": [r'"brand"\s*:\s*"(.*?)"', r'"brandName"\s*:\s*"(.*?)"'],
    "gender": [r'"gender"\s*:\s*"(.*?)"'],
    "count_per_pack": [r'"countPerPack"\s*:\s*"(.*?)"', r'"count_per_pack"\s*:\s*"(.*?)"', r'"packageQuantity"\s*:\s*"(.*?)"'],
    "multipack": [r'"multiPack"\s*:\s*"(.*?)"', r'"multipack"\s*:\s*"(.*?)"'],
    "total_count": [r'"totalCount"\s*:\s*"(.*?)"', r'"total_count"\s*:\s*"(.*?)"', r'"itemCount"\s*:\s*"(.*?)"'],
    "size": [r'"size"\s*:\s*"(.*?)"', r'"sizeDescription"\s*:\s*"(.*?)"'],
    "serving_size": [r'"servingSize"\s*:\s*"(.*?)"', r'"serving_size"\s*:\s*"(.*?)"', r'"servingSizeDescription"\s*:\s*"(.*?)"'],
    "color": [r'"color"\s*:\s*"(.*?)"', r'"colour"\s*:\s*"(.*?)"'],
    "flavour": [r'"flavor"\s*:\s*"(.*?)"', r'"flavour"\s*:\s*"(.*?)"', r'"taste"\s*:\s*"(.*?)"'],
    "product_form": [r'"productForm"\s*:\s*"(.*?)"', r'"dosageForm"\s*:\s*"(.*?)"', r'"product_form"\s*:\s*"(.*?)"', r'"form"\s*:\s*"(.*?)"'],
    "ingredient_statement": [
        r'"ingredientStatement"\s*:\s*"(.*?)"', r'"ingredients"\s*:\s*"(.*?)"',
        r'"ingredientList"\s*:\s*"(.*?)"', r'"otherIngredients"\s*:\s*"(.*?)"',
        r'"activeIngredients"\s*:\s*"(.*?)"',
    ],
    "dosage": [r'"dosage"\s*:\s*"(.*?)"', r'"suggestedUse"\s*:\s*"(.*?)"', r'"recommendedDose"\s*:\s*"(.*?)"'],
    "directions": [r'"directions"\s*:\s*"(.*?)"', r'"suggestedUse"\s*:\s*"(.*?)"', r'"usageDirections"\s*:\s*"(.*?)"'],
    "instructions": [r'"instructions"\s*:\s*"(.*?)"', r'"usageInstructions"\s*:\s*"(.*?)"'],
    "stop_use_indications": [r'"stopUseIndications"\s*:\s*"(.*?)"', r'"warnings"\s*:\s*"(.*?)"', r'"warning"\s*:\s*"(.*?)"', r'"caution"\s*:\s*"(.*?)"'],
...
IMAGE_PATH_ALLOW_HINTS = {
    ...
    "secondaryimage", "alternateimage", "alt image", "hero image", "heroimage",
}
...
IMAGE_PATH_BLOCK_HINTS = {
    ...
    "swatch", "flag", "sticker", "promo", "marketing", "thumbnail icon", "registry",
    "subscription", "walmart plus", "seller", "category", "department", "collection",
}

PRODUCT_SCOPE_ALLOW_HINTS = {
    "product", "item", "primaryproduct", "selectedproduct", "selected item", "buybox",
    "productdetails", "product detail", "productinfo", "iteminfo", "usitem", "product data",
    "productpage", "product page", "addtocart", "fulfillmentoptions", "offerinfo",
}

PRODUCT_SCOPE_BLOCK_HINTS = {
    "variant", "variants", "options", "choice", "choices", "swatch", "thumbnail",
    "recommend", "recommended", "similar", "related", "sponsored", "ad", "ads",
    "registry", "footer", "header", "navigation", "nav", "reviews", "review",
    "seller", "cegateway", "events", "event", "module", "carousel", "subscription",
    "upsell", "crosssell", "cross sell", "brandshop", "campaign", "department",
}

SCALAR_BAD_VALUE_PATTERNS = [
    r"^true$",
    r"^false$",
    r"^cegateway$",
    r"^events registry$",
...
PRODUCT_FORM_CHOICES = [
    "Capsules", "Gummies", "Tablets", "Softgels", "Powder", "Liquid", "Chews", "Caplets",
    "Cream", "Gel", "Ointment", "Spray", "Drops", "Patch", "Patches", "Lotion", "Bar",
    "Bars", "Tea", "Drink Mix", "Lozenges", "Packet", "Packets", "Sachet", "Sachets",
]
...
MAJOR_SECTION_PATTERNS = [
    ...
    r"^warranty$",
    r"^warnings?$",
    r"^stop use indications?$",
    r"^similar items.*$",
    r"^based on what customers bought$",
    r"^how do you want your item.*$",
]
...
def candidate_conflicts_with_field(field_key: str, candidate_key: str) -> bool:
    normalized_key = normalize_label(candidate_key)
    return any(token in normalized_key for token in FIELD_CONFLICT_TOKENS.get(field_key, []))


def merge_candidate_maps(*maps: Dict[str, List[str]]) -> Dict[str, List[str]]:
    merged: Dict[str, List[str]] = {}
    for candidate_map in maps:
        for label, values in candidate_map.items():
            for value in values:
                add_candidate(merged, label, value)
    return merged


def explicit_aliases_for_field(field_key: str) -> List[str]:
    return FIELD_ALIASES.get(field_key, [FIELD_LABELS.get(field_key, field_key)])
...
def normalize_image_url(url: str) -> str:
    text = canonicalize_url(url)
    text = re.sub(r'([?&])odn(?:Height|Width|Bg)=[^&]+', '', text, flags=re.I)
    text = re.sub(r'[?&]+$', '', text)
    if 'walmartimages' in text.lower():
        text = text.split('?', 1)[0]
    return text


def value_matches_any(text: str, patterns: Iterable[str]) -> bool:
...
def extract_quantity_markers(text: str) -> set[str]:
    ...
    markers = set()
    for pattern in [
        r'\b\d+(?:\.\d+)?\s*(?:count|ct|capsules?|tablets?|gummies|softgels?|pack(?:\s+of\s+\d+)?|pieces|servings?)\b',
        r'\bpack\s+of\s+\d+\b',
    ]:
        for match in re.finditer(pattern, cleaned, flags=re.I):
            markers.add(clean_text(match.group(0)).lower())
    return markers


def title_similarity_score(left: str, right: str) -> float:
    left_tokens = {token for token in re.findall(r'[A-Za-z0-9]+', clean_text(left).lower()) if len(token) > 1}
    right_tokens = {token for token in re.findall(r'[A-Za-z0-9]+', clean_text(right).lower()) if len(token) > 1}
...
def quantity_markers_conflict(left: str, right: str) -> bool:
    left_markers = extract_quantity_markers(left)
    right_markers = extract_quantity_markers(right)
    return bool(left_markers and right_markers and left_markers.isdisjoint(right_markers))


def is_probable_product_record(obj: Dict[str, Any], path_parts: List[str]) -> bool:
    path_text = ' '.join(path_parts)
    if any(token in path_text for token in PRODUCT_SCOPE_BLOCK_HINTS):
        return False
    normalized_keys = {normalize_label(key) for key in obj.keys()}
    signal_keys = {
        'productname', 'name', 'title', 'description', 'shortdescription', 'longdescription',
        'imageinfo', 'allimages', 'mainimage', 'mainimageurl', 'usitemid', 'itemid', 'brand',
    }
    if len(normalized_keys & signal_keys) < 2:
        return False
    if 'name' not in normalized_keys and 'productname' not in normalized_keys and 'title' not in normalized_keys:
        if 'usitemid' not in normalized_keys and 'itemid' not in normalized_keys:
            return False
    return True
...
def collect_product_records(obj: Any, records: List[Tuple[List[str], Dict[str, Any]]], path_parts: Optional[List[str]] = None) -> None:
    ...
    if isinstance(obj, dict):
        if is_probable_product_record(obj, path_parts):
            records.append((path_parts, obj))
        for key, value in obj.items():
            collect_product_records(value, records, path_parts + [normalize_label(str(key))])
    elif isinstance(obj, list):
        for item in obj:
            collect_product_records(item, records, path_parts)


def record_lookup_values(record: Any, aliases: Iterable[str], path_parts: Optional[List[str]] = None) -> List[str]:
    path_parts = path_parts or []
    values: List[str] = []
    ...
    if isinstance(record, dict):
        for key, value in record.items():
            ...
            next_path = path_parts + [normalized_key]
            if any(token in ' '.join(next_path) for token in PRODUCT_SCOPE_BLOCK_HINTS):
                continue
            if normalized_key in normalized_aliases and not isinstance(value, (dict, list)):
                cleaned = normalize_json_text(str(value))
                if cleaned:
                    values.append(cleaned)
            values.extend(record_lookup_values(value, aliases, next_path))
    elif isinstance(record, list):
        for item in record:
            values.extend(record_lookup_values(item, aliases, path_parts))
    return values
...
def score_product_record(path_parts: List[str], record: Dict[str, Any], page_title: str, item_id: str) -> float:
    ...
    if name:
        ...
        if quantity_markers_conflict(name, page_title):
            score -= 80
    record_item_id = first_non_empty([
        record.get('usItemId', ''),
        record.get('itemId', ''),
        record.get('us_item_id', ''),
        record.get('item_id', ''),
    ])
    if item_id and record_item_id and clean_text(record_item_id) == item_id:
        score += 100
    if any(token in path_text for token in PRODUCT_SCOPE_ALLOW_HINTS):
        score += 25
    if record_lookup_values(record, ['imageInfo', 'allImages', 'mainImage', 'mainImageUrl']):
        score += 20
    if record_lookup_values(record, ['description', 'shortDescription', 'longDescription']):
        score += 12
    return score
...
def walk_product_record(
    ...
    if isinstance(obj, list):
        for item in obj:
            walk_product_record(item, candidate_map, image_urls, current_key=current_key, path_parts=path_parts)
        return
    if obj is None:
        return
    ...
    if not text:
        return
    if current_key and current_key not in {'url', 'link', 'href'}:
        add_candidate(candidate_map, current_key, text)
    normalized_url = normalize_image_url(text)
    if is_probable_image_url(normalized_url) and path_looks_like_product_image(path_parts):
...
def looks_like_bad_value(value: str) -> bool:
    ...
    if any(re.search(pattern, lowered, flags=re.I) for pattern in GENERIC_BAD_VALUE_PATTERNS):
        return True
    if any(re.search(pattern, lowered, flags=re.I) for pattern in SCALAR_BAD_VALUE_PATTERNS):
        return True
...
def extract_json_candidates_from_soup(soup: BeautifulSoup, resolved_url: str = '', page_title: str = '') -> Tuple[Dict[str, List[str]], List[str]]:
    records: List[Tuple[List[str], Dict[str, Any]]] = []
    for script in soup.find_all('script'):
        for parsed in iter_json_payloads_from_script(script):
            try:
                collect_product_records(parsed, records)
            except Exception:
                continue
    ...
    if item_id:
        exact_records = []
        for path_parts, record in records:
            record_item_id = first_non_empty([
                record.get('usItemId', ''),
                record.get('itemId', ''),
                record.get('us_item_id', ''),
            ...
            if clean_text(record_item_id) == item_id:
                exact_records.append((path_parts, record))
        candidate_records = exact_records or records
    else:
        candidate_records = records

    scored_records: List[Tuple[float, List[str], Dict[str, Any]]] = []
    for path_parts, record in candidate_records:
        score = score_product_record(path_parts, record, page_title, item_id)
        scored_records.append((score, path_parts, record))
    scored_records.sort(key=lambda item: item[0], reverse=True)

    candidate_map: Dict[str, List[str]] = {}
    image_urls: List[str] = []
    if not scored_records:
        return candidate_map, image_urls

    best_score = scored_records[0][0]
    selected_records = [item for item in scored_records if item[0] >= max(best_score - 30, 40)][:5]
    for idx, (_, _, record) in enumerate(selected_records):
        local_map: Dict[str, List[str]] = {}
        local_images: List[str] = []
        walk_product_record(record, local_map, local_images)
        candidate_map = merge_candidate_maps(candidate_map, local_map)
        if idx == 0 and local_images:
            image_urls = dedupe_keep_order(local_images)
    return candidate_map, image_urls
...
def extract_pairs_from_section(lines: List[str], heading_patterns: Iterable[str], max_scan_lines: int = 150) -> Dict[str, List[str]]:
    candidate_map: Dict[str, List[str]] = {}
    start_idx = find_heading_index(lines, heading_patterns)
    if start_idx is None:
        return candidate_map
    section_lines = collect_section_lines(lines, start_idx, max_scan_lines=max_scan_lines)
    idx = 0
    while idx < len(section_lines):
        line = clean_text(section_lines[idx])
        if not line:
            idx += 1
            continue
        if ':' in line and len(line.split(':', 1)[0]) <= 40:
            ...
            if looks_like_label_text(label) and clean_text(value):
                add_candidate(candidate_map, label, clean_text(value))
                idx += 1
                continue
        label = strip_bullet_prefix(line)
        if looks_like_label_text(label):
            next_line = clean_text(section_lines[idx + 1]) if idx + 1 < len(section_lines) else ''
            if next_line and not looks_like_label_text(next_line) and not is_generic_section_heading(next_line):
                add_candidate(candidate_map, label, next_line)
                idx += 2
                continue
        idx += 1
    return candidate_map
...
def build_explicit_alias_candidate_map(lines: List[str]) -> Dict[str, List[str]]:
    candidate_map: Dict[str, List[str]] = {}
    normalized_aliases = {
        field_key: [normalize_label(alias) for alias in explicit_aliases_for_field(field_key)]
    ...
    for idx, line in enumerate(lines):
        ...
        if not text or looks_like_bad_value(text):
            continue
        next_line = clean_text(lines[idx + 1]) if idx + 1 < len(lines) else ''
        norm_line = normalize_label(text)
        colon_label = ''
        colon_value = ''
        if ':' in text and len(text.split(':', 1)[0]) <= 60:
            colon_label, colon_value = text.split(':', 1)
            colon_label = normalize_label(colon_label)
            colon_value = clean_multiline_text(colon_value)
        for field_key, aliases in normalized_aliases.items():
            matched_label = ''
            if colon_label and any(candidate_key_match_score(colon_label, alias) >= 94 for alias in aliases):
                matched_label = colon_label
                value = colon_value
            elif any(candidate_key_match_score(norm_line, alias) >= 96 for alias in aliases):
                matched_label = norm_line
                value = next_line
            elif any(norm_line.startswith(alias + ' ') for alias in aliases if alias):
                matched_label = norm_line
                prefix_len = min((len(alias) for alias in aliases if norm_line.startswith(alias + ' ')), default=0)
                value = clean_text(text[prefix_len:].strip(' :-'))
            else:
                continue
            if candidate_conflicts_with_field(field_key, matched_label):
                continue
            value = sanitize_field_value(field_key, value)
            if value:
                add_candidate(candidate_map, FIELD_LABELS.get(field_key, field_key), value)
    return candidate_map


def build_line_candidate_map(lines: List[str]) -> Dict[str, List[str]]:
    candidate_map: Dict[str, List[str]] = build_explicit_alias_candidate_map(lines)
    for key, patterns in SECTION_PATTERNS.items():
        section_text = extract_section_text(lines, patterns, max_scan_lines=64)
        if section_text:
            add_candidate(candidate_map, FIELD_LABELS.get(key, key), section_text)
    for section_patterns in (
        [r'^specs?$', r'^specifications?$', r'^details?$'],
        [r'^ingredients?$'],
        [r'^directions?$', r'^suggested use$'],
        [r'^indications?$', r'^warnings?$', r'^warning$', r'^cautions?$'],
        [r'^dosage$'],
    ):
        section_pairs = extract_pairs_from_section(lines, section_patterns, max_scan_lines=180)
        for label, values in section_pairs.items():
            for value in values:
...
def build_dom_candidate_map(soup: BeautifulSoup) -> Dict[str, List[str]]:
    ...
    scope_selectors = [
        ...
        '[data-testid*="about"]',
        '[data-automation-id*="spec"]',
        '[data-automation-id*="detail"]',
        '[itemprop="brand"]',
        '[itemprop="description"]',
        '[aria-label*="spec" i]',
        ...
        for dt in root.select('dt'):
            ...
            if dd:
                ...
                if looks_like_label_text(label) and value:
                    add_candidate(candidate_map, label, value)
        root_lines = parse_visible_lines(root.get_text('\n'))
        candidate_map = merge_candidate_maps(candidate_map, build_explicit_alias_candidate_map(root_lines))
        for node in root.select('[data-testid], [aria-label], li, p, span, div'):
...
def lookup_candidate(
    candidate_maps: Iterable[Dict[str, List[str]]],
    ...
    for candidate_map in candidate_maps:
        for key, values in candidate_map.items():
            if field_key and candidate_conflicts_with_field(field_key, key):
                continue
            for alias in normalized_aliases:
                ...
                if score < min_score:
                    continue
                usable_values = [value for value in values if not looks_like_bad_value(value)]
...
def sanitize_simple_choice(value: str) -> str:
    text = clean_text(value)
    if not text or looks_like_bad_value(text):
        return ''
    lowered = text.lower()
    if 'selected,' in lowered or re.search(r',\s*\$[^,]+$', text):
        return ''
    text = re.sub(r'\s+', ' ', text).strip(' ,')
    if looks_like_bad_value(text):
        return ''
    return text


def sanitize_field_value(field_key: str, value: str, title: str = '', description: str = '') -> str:
    text = clean_multiline_text(value)
    if not text or looks_like_bad_value(text):
        return ''
    if field_key in {'brand', 'gender', 'color', 'flavour', 'product_form', 'count_per_pack', 'multipack', 'total_count', 'size', 'serving_size'}:
        text = sanitize_simple_choice(text)
        if len(text) > 120:
            return ''
        if field_key == 'size' and re.fullmatch(r'\d{4,}', text):
            return ''
        return text
    if field_key in LONG_TEXT_FIELDS or field_key in {'directions', 'instructions', 'stop_use_indications'}:
        text = clean_multiline_text(text)
        if len(text) < 10 or looks_like_bad_value(text):
            return ''
        return text
    return sanitize_simple_choice(text)


def _group_image_segments(urls: List[str]) -> List[List[str]]:
    segments: List[List[str]] = []
    current: List[str] = []
    for url in urls:
        lower_url = url.lower()
        if '/seo/' in lower_url and current:
...
def _score_image_segment(segment: List[str], title: str) -> float:
    if not segment:
        return -999.0
    title_markers = extract_quantity_markers(title)
    first_url = segment[0].lower()
    score = min(len(segment), 10) * 3
    if '/seo/' in first_url:
        segment_markers = extract_quantity_markers(first_url.replace('-', ' '))
        if title_markers and segment_markers:
            if title_markers & segment_markers:
                score += 120
            elif title_markers.isdisjoint(segment_markers):
                score -= 140
        elif title_markers and not segment_markers:
            score -= 10
    return score


def select_best_image_urls(urls: List[str], title: str) -> List[str]:
    ...
    for raw in urls:
        ...
        if not is_probable_image_url(url):
            continue
        lower_url = url.lower()
        if any(token in lower_url for token in ['logo', 'icon', 'sprite', 'placeholder', 'rating', 'review', 'sponsored', 'related', 'similar', 'variant', 'swatch', 'thumbnail']):
            continue
        if '/seo/' in lower_url and title_markers:
            url_markers = extract_quantity_markers(lower_url.replace('-', ' '))
            if url_markers and url_markers.isdisjoint(title_markers):
                continue
        cleaned_urls.append(url)
    cleaned_urls = dedupe_keep_order(cleaned_urls)
    if not cleaned_urls:
        return []

    segments = _group_image_segments(cleaned_urls)
    if len(segments) > 1:
        best_segment = max(segments, key=lambda seg: _score_image_segment(seg, title))
        best_score = _score_image_segment(best_segment, title)
        if best_score > -50:
            cleaned_urls = dedupe_keep_order(best_segment)
    ...
    for url in cleaned_urls:
        ...
        if '/seo/' in lower_url and title_markers:
            url_markers = extract_quantity_markers(lower_url.replace('-', ' '))
            current_title_match = not url_markers or not url_markers.isdisjoint(title_markers)
        if current_title_match:
            filtered_urls.append(url)
    return dedupe_keep_order(filtered_urls)

def extract_image_links(soup: BeautifulSoup, title: str, json_image_urls: List[str]) -> List[str]:
    preferred_urls: List[str] = []
    for url in json_image_urls:
        preferred_urls.append(normalize_image_url(url))

    strict_urls = select_best_image_urls(preferred_urls, title)
    if strict_urls:
        return strict_urls
    ...
    for attrs in [
        ...
        if tag and tag.get('content'):
            meta_urls.append(normalize_image_url(tag.get('content', '')))
    return select_best_image_urls(meta_urls, title)
...
def extract_field_value(
    ...
    if field_key in SECTION_PATTERNS:
        section_text = extract_section_text(lines, SECTION_PATTERNS[field_key], max_scan_lines=64)
    strict_min = 92 if field_key in {'count_per_pack', 'multipack', 'total_count', 'size', 'serving_size'} else 88
    relaxed_min = 84 if field_key in {'count_per_pack', 'multipack', 'total_count', 'size', 'serving_size'} else 76
    strict_candidate = lookup_candidate(
        candidate_maps,
        aliases,
        prefer_long=field_key in LONG_TEXT_FIELDS,
        min_score=strict_min,
        field_key=field_key,
    )
    regex_value = regex_extract_first(html, FIELD_REGEX_PATTERNS.get(field_key, []))
    relaxed_candidate = lookup_candidate(
        candidate_maps,
        aliases,
        prefer_long=field_key in LONG_TEXT_FIELDS,
    ...
    if field_key in LONG_TEXT_FIELDS or field_key in {'directions', 'instructions', 'stop_use_indications'}:
        value = first_non_empty([section_text, strict_candidate, regex_value, relaxed_candidate])
    else:
        value = first_non_empty([strict_candidate, regex_value, section_text, relaxed_candidate])
    cleaned = sanitize_field_value(field_key, value, title=title, description=description)
...
def scrape_listing_from_html(html: str, selected_field_keys: List[str], resolved_url: str = '') -> ExtractionBundle:
    ...
    preliminary_title = extract_title_from_sources(soup, {}, {})
    json_candidates, json_image_urls = extract_json_candidates_from_soup(soup, resolved_url=resolved_url, page_title=preliminary_title)
    ...
    candidate_maps = [dom_candidates, json_candidates, line_candidates]
    ...
    if need_title or need_scalar_fields or need_images:
    ...
    if need_description or need_scalar_fields:
        description = first_non_empty(
            [
                ...
                lookup_candidate([line_candidates, json_candidates], ['long description', 'product description', 'short description', 'description'], prefer_long=True),
    ...
    if need_bullets:
        bullets = dedupe_keep_order(
            extract_bullets_from_lines(lines) + extract_bullets_from_candidate_maps([line_candidates, json_candidates])
        )[:MAX_DEEP_BULLETS]
    if need_images:
        images = extract_image_links(soup, title, json_image_urls)
...
def build_output_columns(
    ...
    if 'brand' in selected_field_keys:
...
def build_output_row(
    ...
    if 'brand' in selected_field_keys:
        row['Brand'] = bundle.field_values.get('brand', '') or NOT_FOUND_TEXT
